...
import com.example.projetm1.GraphicOverlay.Graphic
...
class GraphicOverlay(context: Context?, attrs: AttributeSet?) :
    ...
    private val graphics: MutableList<com.example.projetm1.GraphicOverlay.Graphic> =
        ArrayList<com.example.projetm1.GraphicOverlay.Graphic>()
    ...
    abstract class Graphic(private val overlay: GraphicOverlay) {
        ...
        private fun constrainToRange(y: Int, min: Int, max: Int): Int{
            return if ( y < min ){
                min
            } else if( y > max ){
                max
    ...
    fun add(graphic: com.example.projetm1.GraphicOverlay.Graphic) {
        synchronized(lock) { graphics.add(graphic) }
    }

    /** Removes a graphic from the overlay.  */
    fun remove(graphic: com.example.projetm1.GraphicOverlay.Graphic) {
    ...
    private fun checkState(expression: Boolean, error: String){
        if (!expression){
